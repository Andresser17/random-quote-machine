import "./App.css";
import { Component } from "react";

class App extends Component {
  constructor(props) {
    super(props);
    this.state = {
      i: 0,
      quotes: [
        {
          author: "Vince Lombardi",
          quote: "Winning isn’t everything, but wanting to win is.",
        },
        {
          author: "Sir Claus Moser",
          quote: "Education costs money. But then so does ignorance.",
        },
        {
          author: "George Eliot",
          quote: "It is never too late to be what you might have been.",
        },
        {
          author: "Jesse Owens",
          quote:
            "The battles that count aren’t the ones for gold medals. The struggles within yourself–the invisible battles inside all of us–that’s where it’s at.",
        },
        {
          author: "Plato",
          quote:
            "We can easily forgive a child who is afraid of the dark; the real tragedy of life is when men are afraid of the light.",
        },
        {
          author: "Norman Vaughan",
          quote: "Dream big and dare to fail.",
        },
      ],
      colors: [
        "red",
        "green",
        "blue",
        "orange",
        "grey"
      ]
    };

    // Binding
    this.changeQuote = this.changeQuote.bind(this);
  }

  changeQuote() {
    const i = this.state.i;
    const quotes = this.state.quotes;

    if (i + 2 <= quotes.length) {
      this.setState((state) => {
        return {
          i: state.i + 1,
        };
      });

    } else {
      this.setState((state) => {
        return {
          i: 0,
        };
      });
    }
  }

  render() {
    const i = this.state.i;
    const quote = this.state.quotes[i].quote;
    const author = this.state.quotes[i].author;
    const tweet = `https://twitter.com/intent/tweet?hashtags=quotes&related=freecodecamp&text="${quote}" ${author}`;
    const color = this.state.colors[Math.floor(Math.random()*this.state.colors.length)];

    return (
      <div className={color} id="quote-box">
        <p id="text">"{quote}"</p>
        <span id="author">- {author}</span>
        <div class="quote-options">
          <a id="tweet-quote" href={tweet} target="_blank" rel="noreferrer">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="16"
              fill="currentColor"
              class="bi bi-twitter"
              viewBox="0 0 16 16"
            >
              <path d="M5.026 15c6.038 0 9.341-5.003 9.341-9.334 0-.14 0-.282-.006-.422A6.685 6.685 0 0 0 16 3.542a6.658 6.658 0 0 1-1.889.518 3.301 3.301 0 0 0 1.447-1.817 6.533 6.533 0 0 1-2.087.793A3.286 3.286 0 0 0 7.875 6.03a9.325 9.325 0 0 1-6.767-3.429 3.289 3.289 0 0 0 1.018 4.382A3.323 3.323 0 0 1 .64 6.575v.045a3.288 3.288 0 0 0 2.632 3.218 3.203 3.203 0 0 1-.865.115 3.23 3.23 0 0 1-.614-.057 3.283 3.283 0 0 0 3.067 2.277A6.588 6.588 0 0 1 .78 13.58a6.32 6.32 0 0 1-.78-.045A9.344 9.344 0 0 0 5.026 15z" />
            </svg>
          </a>
          <button id="new-quote" onClick={this.changeQuote}>
            New Quote
          </button>
        </div>
      </div>
    );
  }
}

export default App;
